...
class Mp4Frag extends Transform {
  ...
  constructor(options) {
    ...
    if (options) {
      ...
      if (options.hasOwnProperty('preBufferListSize')) {
        const preBufferListSize = parseInt(options.preBufferListSize);
        if (isNaN(preBufferListSize)) {
          this._preBufferListSize = 0;
        } else if (preBufferListSize > 60) {
          this._preBufferListSize = 60;
        } else {
          this._preBufferListSize = preBufferListSize;
        }
        this._preBufferList = [];
      }
    }
    this.newSegmentSubject = new rxjs.Subject();
    this.newSegmentObservable = this.newSegmentSubject.asObservable();
    this._parseChunk = this._findFtyp;
    this._camId = options.camId;

    this.stallDetectionObservable = this.newSegmentSubject.asObservable().pipe(timeout(10000)).subscribe(val => {

    }, err => {
      this.stallDetectionObservable.unsubscribe();
      this.emit('error', new Error('No new segment received for 10 secs.  Stream stalled')) ;
    });

    // set source on pipe so we can re-pipe automatically
    // this.source = null;
    // this.on('pipe', (source) => {
    //     this.source = source;
    // });
  ...
  _setSegment(chunk) {
    ...
    this._duration = currentTime - this._timestamp;

    const newSeg = {
      sequence: ++this._sequence,
      name: `${this._hlsBase}${this._sequence}.m4s`,
      segment: this._segment,
      duration: Math.max((currentTime - this._timestamp) / 1000),
      duration_ms: this._duration,
      timestamp: this._timestamp - 300 // subtract 300ms for camera latency. This differs slightly per camera, but this is usually good enough to 
                                       // get a valid video timestamp
    }
    if (this._hlsList) {
      this._hlsList.push(newSeg);
      while (this._hlsList.length > this._hlsListSize) {
        this._hlsList.shift();
      }

      const biggestDuration = Math.round(Math.max(...this._hlsList.map(item => {return item.duration} )));
      let m3u8 = '#EXTM3U\n';
      m3u8 += '#EXT-X-VERSION:7\n';
      //m3u8 += '#EXT-X-ALLOW-CACHE:NO\n';
      m3u8 += `#EXT-X-TARGETDURATION:${biggestDuration}\n`;
      m3u8 += `#EXT-X-MEDIA-SEQUENCE:${this._hlsList[0].sequence}\n`;
      m3u8 += `#EXT-X-MAP:URI="init-${this._hlsBase}.mp4"\n`;
      for (let i = 0; i < this._hlsList.length; i++) {
        m3u8 += `#EXTINF:${this._hlsList[i].duration.toFixed(6)},\n`;
        m3u8 += `#EXT-X-PROGRAM-DATE-TIME:${new Date(this._hlsList[i].timestamp).toISOString()}\n`
    ...
    if (this._preBufferList) {
      this._preBufferList.push(this.newSeg);
      while (this._preBufferList.length > this._preBufferListSize) {
        this._preBufferList.shift();
      }
    }
     ...
     * @type {Event}
     * @property {neSeg} newSeg - [Mp4Frag.segment]{@link Mp4Frag#segment}
     */
    this.emit('segment', newSeg);
    this.newSegmentSubject.next(newSeg)
